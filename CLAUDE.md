...
13. **R.S.V.P.** — "참석 의사 전달", 설명 2줄, `참석의사 전달하기` 버튼 → **전체 화면 흐림 배경 폼**(`Modal variant="full"`, 참고 mZdJd64x2r): 참석 가능/불가 카드(체크 원), 성함 밑줄 입력 + 신랑측/신부측 라디오, 추가 인원 원형 ±(본인 제외, headcount = 1+추가), 개인정보 동의 카드, 다 채워야 버건디로 활성화되는 버튼. **진입 즉시 뜨는 하단 시트 팝업**(`RsvpPopup.tsx`: 아이콘 · 안내 · 일시/장소/주소 행 · `참석의사 전달하기` 하나. 2026-09-10 사용자 요청으로 '오늘 하루 보지 않기' 제거, 지연 없이 첫 렌더링부터 열림, 매 방문 표시) 포함. **구현됨**, 2026-09-10 헤드리스로 제출까지 검증.
