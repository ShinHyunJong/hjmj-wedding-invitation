...
- 배포 도메인: **https://hyunjong-minji-wedding.vercel.app** (2026-09-15). 카카오 Web 플랫폼 · 카카오톡 공유 링크 관리 · NCP Web 서비스 URL 등록 완료. Vercel 함수 리전은 iad1 → icn1 권장, ADMIN_TOKEN 미설정. 커스텀 도메인 붙이면 같은 세 곳 + NEXT_PUBLIC_SITE_URL 갱신.
...
12. **Account** — eyebrow `ACCOUNT`, "마음 전하실 곳", 안내 3줄. `신랑측 계좌번호` / `신부측 계좌번호` 그룹 — **기본 펼침**(2026-09-14, 탭하면 접힘). 행마다 예금주, 은행 · 계좌번호, 오른쪽 `복사` 알약 버튼(복사 후 "복사됨"), 카카오페이 링크 있으면 노란 원형 버튼.
...
17. **Ending** — 전폭 사진(2:3, `FEATURED.ending` + `endingPosition` + `endingTone`) 위에 시. 상단은 흰색으로 페이드. `endingTone`: dark = 하단 어둡게 + 흰 글씨, light = 하단 흰색 스밈 + 차콜 글씨(밝은 사진용, 2026-09-14 추가). 현재 YS_05644(정원 샴페인 컷, dark — 전체에 은은한 검은 오버레이 + 하단 진하게, 흰 글씨). 글은 세리프 한글 · 자간 0.14em · 줄간격 2.2, 아래에 출처. 문구는 `wedding.ending` (지금은 이수동 「사랑가」, **두 사람 문구로 교체 여부 확인**).
...
- **카카오 키**: `.env`의 `KAKAO_JS_KEY` (JavaScript 키, gitignore 됨). `next.config.ts`가 `NEXT_PUBLIC_KAKAO_JS_KEY`로 노출한다. **키나 값을 채팅 · 문서 · 로그에 절대 출력하지 않는다** (`.env` 를 볼 때는 `sed -E 's/=(.{3}).*/=\1…/'` 로 마스킹).
  ...
  - **공유 메시지의 버튼 링크**는 별도 메뉴 **[제품 설정 → 카카오톡 공유 → 제품 링크 관리 → 웹 도메인]** 에 등록된 도메인만 허용된다(플랫폼 Web 목록과 다름). 미등록이면 카카오가 그 목록의 '기본' 도메인(회사 사이트)으로 링크를 바꿔 보낸다 — 2026-09-15 증상, `https://hyunjong-minji-wedding.vercel.app` 등록으로 해결. 기본 도메인은 여전히 회사 도메인. 공유 링크 origin 은 `NEXT_PUBLIC_SITE_URL` 로 고정 가능.
